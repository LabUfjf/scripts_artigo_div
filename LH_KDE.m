...
for i = 1:length(opt.SG)+1;
    ...
    for cvtr = 1:N.BLOCKS;
        ...
        for nv = 1:4;
            ...
            [x.tr.sg,y.tr.sg] = kernelClean(DATASG.TR,2000,opt.SG(nv),1);
            [x.tr.bg,y.tr.bg] = kernelClean(DATABG.TR,2000,opt.BG(nv),1);
            else
...
%             subplot(2,2,nv);plot(x.tr.sg,y.tr.sg,'k',x.tr.bg,y.tr.bg,'r'); hold on            
            ...
            disp(['[KDE][BIN=' num2str((i/[length(opt.SG))+1]*100) '%][CV='  num2str((cvtr/N.BLOCKS)*100) '%][VAR='  num2str((nv/4)*100) '%]'])
