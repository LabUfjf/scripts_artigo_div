...
for name = {'Uniform','Gaussian','Bimodal','Trimodal','Rayleigh','Logn','Gamma','Laplace'};
    ...
    for i=1:length(vGRID);
        ...
        DA(i)=abs(area2d(DATA.sg.pdf.truth.x,DATA.sg.pdf.truth.y)-DATA.sg.Integral); %mise
        %       DA(i)=sum((DATA.sg.pdf.truth.x-DATA.sg.pdf.truth.y-DATA.sg.Integral).^2); %area
    ...
    %     vDA(j) = mean(DA);
    plot(vGRID,DA,['-' cl(j)])
    v(j)= interp1(vGRID,DA,10^5,'linear');
...
legend('Uniform','Gaussian','Bimodal','Trimodal','Rayleigh','Logn','Gamma','Laplace')
% pause
