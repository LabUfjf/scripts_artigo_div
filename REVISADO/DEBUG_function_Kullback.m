...
%             Q =  P + setup.RANGE.NOISE(sd(j))*randn(1,length(P));
%                 Q = sort(Q);
        else
            P =  Q + (Q/max(Q)).*f.norm(sd(j)).*randn(1,length(Q));
            P(P<0) = 0;            
%              Q =  P + (P/max(P)).*f.norm(sd(j)).*randn(1,length(P));
%              Q = sort(Q);
%             Q(Q<0) = 0;
        end
        %=========================================================================
...
subplot(2,1,1); legend({'P[1]','log(P/Q)[1]','P[2]','log(P/Q)[2]','P[3]','log(P/Q)[3]','Max/Min'},'Location','best');xlabel('Probability'); ylabel('Amplitude')
subplot(2,1,2); legend({'Plog(P/Q)[1]','Plog(P/Q)[2]','Plog(P/Q)[3]'},'Location','best'); xlabel('Probability'); ylabel('Amplitude')
set(gcf,'units','points','position',[10,10,550,400])
