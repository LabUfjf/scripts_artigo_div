...
for name = {'Gaussian','Bimodal','Trimodal','Rayleigh','Logn','Gamma','Laplace'};
    ...
    saveas(gcf,[pwd '\DESENVOLVIMENTO\HIST[BIN][' name{1} ']'],'fig');
    saveas(gcf,[pwd '\DESENVOLVIMENTO\HIST[BIN][' name{1} ']'],'eps');
    saveas(gcf,[pwd '\DESENVOLVIMENTO\HIST[BIN][' name{1} ']'],'png');
    close(1)
    ...
    set(gca,'Gridlinestyle',':')
    % set(gcf, 'Position', get(0, 'Screensize'));
    saveas(gcf,[pwd '\DESENVOLVIMENTO\HIST[AREA][' name{1} ']'],'fig');
    saveas(gcf,[pwd '\DESENVOLVIMENTO\HIST[AREA][' name{1} ']'],'eps');
    saveas(gcf,[pwd '\DESENVOLVIMENTO\HIST[AREA][' name{1} ']'],'png');
    close(2)
    
    save([pwd '\DESENVOLVIMENTO\HIST[BIN][' name{1} ']'],'BIN');
    save([pwd '\DESENVOLVIMENTO\HIST[AREA][' name{1} ']'],'AREA');
