...
[setup] = IN('Gaussian','sg','normal','dist','linear','bypass',1,1e4,1000,1);   % Definir os Parâmetros Iniciais
...
subplot(2,2,1);legend('(P+Q)/2[1]','(P+Q)/2[2]','(P+Q)/2[3]');xlabel('Probability'); ylabel('Amplitude'); grid minor; axis tight;
subplot(2,2,2);legend('log((P+Q)/(2\surdPQ))[1]','log((P+Q)/(2\surdPQ))[2]','log((P+Q)/(2\surdPQ))[3]');xlabel('Probability'); ylabel('Amplitude');grid minor; axis tight
subplot(2,2,3:4);legend('((P+Q)/2)log((P+Q)/(2\surdPQ))[1]','((P+Q)/2)log((P+Q)/(2\surdPQ))2]','((P+Q)/2)log((P+Q)/(2\surdPQ))[3]');xlabel('Probability'); ylabel('Amplitude'); grid minor; axis tight
