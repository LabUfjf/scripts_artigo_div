function [pdf,grid]=akde1d(X,m,grid,gam)
...
if (nargin<3)|isempty(grid) % failing to provide grid
    grid=(MIN:scaling/(m-1):MAX)';
end
mesh=bsxfun(@minus,grid,MIN);mesh=bsxfun(@rdivide,mesh,scaling);
if nargin<4 % failing to provide speed/accuracy tradeoff
    gam=ceil(n^(1/3))+20;
end
% end preprocessing
