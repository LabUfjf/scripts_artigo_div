...
for j = 1:length(sd)
    ...
    for i=1:nt
        ...
        F1(i,:,:)=real(V0);
        %=========================================================================
        V1=real(log(real((P+Q)./real(2*sqrt(P.*Q)))));   % FUNÇÃO B
...
subplot(3,2,1);legend('(P+Q)/2[1]','(P+Q)/2[2]','(P+Q)/2[3]');xlabel('data'); ylabel('Amplitude'); grid minor; axis tight;
subplot(3,2,2);legend('log((P+Q)/(2\surdPQ))[1]','log((P+Q)/(2\surdPQ))[2]','log((P+Q)/(2\surdPQ))[3]');xlabel('data'); ylabel('Amplitude');grid minor; axis tight
subplot(3,2,3:4);legend('((P+Q)/2)log((P+Q)/(2\surdPQ))[1]','((P+Q)/2)log((P+Q)/(2\surdPQ))[2]','((P+Q)/2)log((P+Q)/(2\surdPQ))[3]'); ylabel('Amplitude'); grid minor; axis tight
...
for j =  1:length(sd)
    ...
    for i=1:nt
        ...
        F1(i,:)=real(V0);
        %=========================================================================
        V1=real(log(real((P+Q)./real(2*sqrt(P.*Q)))));
        V1(isnan(V1)|isinf(V1))=0;
        F2(i,:)=real(V1);
        %=========================================================================
...
subplot(2,1,1); legend({'(P+Q)/2[1]','log((P+Q)/(2\surdPQ))[1]','(P+Q)/2[2]','log((P+Q)/(2\surdPQ))[2]','(P+Q)/2[3]','log((P+Q)/(2\surdPQ))[3]','Max/Min'},'Location','best');xlabel('Probability'); ylabel('Amplitude')
subplot(2,1,2); legend({'((P+Q)/2)log((P+Q)/(2\surdPQ))[1]','((P+Q)/2)log((P+Q)/(2\surdPQ))[2]','((P+Q)/2)log((P+Q)/(2\surdPQ))[3]'},'Location','best'); xlabel('Probability'); ylabel('Amplitude')
set(gcf,'units','points','position',[10,10,550,400])
