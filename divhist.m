...
x = sg.evt;
% function [optN, optD, edges, C, N] = divhist(x,N)
...
for i = 1: length(N)
    ...
    xn = linspace(min(x),max(x),N(i));
    [P,xpdf]=hist(CV',xn);
    
    %     P=interp1(xpdf,ypdf,xn,'nearest','extrap');
    
    Q = repmat(mean(P,1),N(i),1);
    
    %     Sorensen(i,:) = [sum(abs(P-Q))./(sum(abs(P+Q)))]/D(i)^2;
    %
    %     RN.ISB=(sum(P-Q))./length(P);
    %     RN.IV=(sum((P-Q).^2))./length(P);
    %     MISE(i,:) = [RN.IV+(RN.ISB).^2];
    v = sum( (P-Q).^2 ,2)/N(i);
    Cs(i,:) = ( 2*Q - v ) / D(i)^2;    % The Cost Function
