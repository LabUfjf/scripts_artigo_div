...
%==========================================================================
% proby=interp1(xhko,yhko,data,'pchip');                       %cálculo das probabilidades dos eventos de kernel
...
ah = area2d(x,y);        %cálculo da área do histograma
fp_pdf=y/ah;              %normalização do histograma pela área(substituir linhas anteriores pelo data_normalized)
fpi=interp1(x,fp_pdf,data,'nearest','extrap');                %cálculo da probabilidade de cada evento
X = linspace(min(data),max(data),nPoint); %range X do kernel
[hi] = hihj(h,1,fpi,n);         % cálculo da banda variável
